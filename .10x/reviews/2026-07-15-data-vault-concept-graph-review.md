...
- **Correct:** Scope is bounded to research. The commit adds one research record and advances only the owning ticket from `open` to `active`; it selects no store, changes no product behavior, and performs no live mutation. This matches the ticket's research-only scope and explicit exclusions (`.10x/tickets/2026-07-15-research-data-vault-concept-graph.md:9-13,34-38`).
