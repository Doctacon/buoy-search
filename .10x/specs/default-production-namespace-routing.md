...
- `--auto-route`: an accepted compatibility affirmation for scripts written before routing became the default;
- `--route-top-k N`: positive integer, default `3`, maximum `10`, valid in automatic mode;
- `--catalog PATH`: override the automatic-routing catalog path for this command;
- repeatable `--namespace ID`: the sole manual namespace-selection override.

Activation and precedence rules:

- with no CLI `--namespace`, retrieval MUST use catalog routing whether or not `--auto-route` is present;
- one or more CLI `--namespace` values MUST bypass catalog/model routing and retain explicit retrieval behavior;
- CLI `--namespace` combined with `--auto-route`, `--route-top-k`, or retrieve `--catalog` MUST fail before model, credential, catalog mutation, or Turbopuffer work because those options request contradictory modes;
- `TURBOPUFFER_NAMESPACE` MUST NOT be read, selected, or warned about by retrieve in either mode;
- `--route-top-k` and `--catalog` MUST work in default automatic mode without `--auto-route`;
...
`buoy retrieve QUERY` without CLI `--namespace` or `--live` MUST:
...
`buoy retrieve QUERY --live` without CLI `--namespace` MUST perform the identical local route first. Only after a non-empty selected route exists may it read `TURBOPUFFER_API_KEY` and construct live retrievers.
...
Given one or more explicit CLI namespaces, when retrieval runs without automatic-only options, then no catalog/model routing path executes and existing output/retrieval semantics remain compatible.

### Dry preview

Given enabled compatible cards and a cached route model, when ordinary dry-run retrieval runs without a CLI namespace, then exactly the top three by default are planned locally with routing metadata and no credential or remote call.

### Environment replacement

Given `TURBOPUFFER_NAMESPACE` and no CLI namespace, when retrieval runs, then the environment value is neither read nor warned about and catalog routing selects the route.
...
- remote namespace discovery, all-visible-namespace fan-out, or namespace-ID inference;
