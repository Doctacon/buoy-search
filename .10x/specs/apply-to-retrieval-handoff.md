...
- Preflight remains local-only: loading non-secret region configuration is allowed, but credentials, embeddings, and Turbopuffer calls remain prohibited.
...
- `preview` MUST omit `--live`; `live` MUST be otherwise identical and append `--live`.
...
```text
buoy retrieve '<query>' --namespace site-example-com-v1 --region gcp-us-central1 --embedding-model BAAI/bge-small-en-v1.5 --embedding-precision float32 --live
```

## Compatibility

- Existing JSON fields MUST remain unchanged; `region` and `retrieval_commands` are additive.
...
Given multiple local plans and no `--plan`, when apply selects the latest valid plan, then text identifies its exact path/source/artifact and all decision counters plus explicit stale intent.
...
Automatic query text, automatic command execution, namespace discovery, multi-namespace commands, per-namespace metadata persistence, retrieval behavior changes, and plan-command retrieval output before apply review.
