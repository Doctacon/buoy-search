...
The compact machine-readable checkpoint is `.10x/evidence/.storage/2026-07-20-c6-python-syntax-pilot-forecast.json` (3,008,084 bytes; whole-file SHA-256 `e09687381d70a4ed15a4fbbdebfdd661d2dbdaef123546a7c6573757ab71b9d8`; embedded canonical payload SHA-256 `64f2b79a4ab48fb02df181d3ec555a3fc1838bea5f816632309028421987c036`). It records every selected path with source hash/size/language, all nine namespaces and deterministic plan/artifact identities, local artifact file hashes, full row/token/chunk/fallback/storage distributions, per-arm multipliers, first-apply diff summaries, and safety/equivalence results. The complete local plan/manifest/chunk artifacts remain outside the repository under `/tmp/buoy-c6-python-syntax-forecast/plans/` for independent review; they were not staged or applied.
...
The forecast is **not approval-ready** because preflight exposed two review-critical risks:

1. Ruff's exactly equivalent ordinary/current-default plan has **2,722 code source final rows across 170 files whose final `section_path` has no parseable `Lines S-E` component**. Internal Markdown-like headings in source/fixture payloads replace the originating range in current generic parsing. The affected path-list SHA-256 is `abf6436f89e794bc8fed9c06ded00a8ffcb71ac98639ea02b12d7c0fc8674d39`; the complete list is in the checkpoint. This preserves ordinary-default equivalence but conflicts with the active spec's statement that control source chunks retain originating-section `Lines S-E` citations. No source repair is in scope here.
2. The treatment plans contain **15,187 rows above 512 approximate tokens** (fixed: 8,065; AST: 7,122), with maxima of 6,524. The active spec explicitly says 80 physical lines is not a model-token guarantee. Approximate counts are not exact BGE tokenizer counts, and no model load was authorized, so provider/model compatibility and an acceptable storage/chunk bound remain unapproved rather than guessed.
...
It challenges treating the counts as a passing approval checkpoint: active-spec control-citation drift exists in Ruff, long treatment rows need an approved compatibility/resource interpretation, and independent review has not yet passed. C6 must remain blocked on those findings, independent review, and the unchanged separate exact write approval.

## Limits

The storage estimate is deterministic local serialized-row bytes plus raw vector bytes, not provider storage, ANN overhead, transfer encoding, billing, or an approved budget. Approximate token counts are not model tokenizer counts. Local plan hashes and artifacts prove generated content, not remote namespace absence. No credentials, Turbopuffer/provider namespace operations, retrieval, catalog/state mutation, embeddings, model downloads, deletes, or defaults occurred. The ignored `.venv` created by the locked local runner is not project state and is not staged.
